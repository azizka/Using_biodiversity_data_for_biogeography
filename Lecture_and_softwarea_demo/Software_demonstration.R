...
#Occurrence to area classification

#read areas
wwf <- readOGR(dsn = "Example_data", layer = "wwf_biomes_simplified")
plot(wwf, col = wwf@data$biomenames)

#Area classification
outp <- SpGeoCod(as.data.frame(dat.cl), wwf, areanames = "biomenames")

#Visualize results
plot(outp)

#barchart for each polygon with the number of species
plot(outp, type = "speciesrichness")

#write results to disk
WriteOut(outp, type = "stats")
WriteOut(outp, type = "BioGeoBEARS")
WriteOut(outp, type = "Nexus")

#Occurrence threshold
outp2<- SpGeoCod(as.data.frame(dat.cl), wwf, areanames = "biomenames", occ.thresh = 10)
plot(outp2, type = "speciesrichness")

outp3<- SpGeoCod(dat.cl, wwf, areanames = "biomenames", occ.thresh = 90)
plot(outp, type = "speciesrichness")
